...
import {FishingMaterialSort} from "@/gql/graphql"
...
type AllowedSortOptions = keyof FishingMaterialSort
  ...
  queryOptions({
    ...
    queryFn: async () =>
      gqlClient.request(query, {
        ...
        order_by: [
          {[sortKey]: sortDirection} as unknown as FishingMaterialSort,
        ],
      }),
  })

export const Route = createFileRoute("/database/(fishing)/fishing_material")({
  loaderDeps: ({
...
export const fishingMaterialBreadcrumbItems: BreadcrumbItem[] = [
  ...databaseBreadcrumbItems,
  {
    label: "Fishing Materials",
    href: "/database/fishing_material",
  },
]
...
function RouteComponent() {
  ...
  const {
    data: {
      all_fishing_materials: {total_count, items},
    },
  } = useSuspenseQuery(
  ...
  return (
    <>
      <PageHeader>
        <PageHeaderBreadcrumbs items={fishingMaterialBreadcrumbItems} />
        <PageTitle title={last(fishingMaterialBreadcrumbItems)!.label} />
      </PageHeader>
      ...
      <PaginationContentWrapper>
        ...
        <GridItemsList items={items}>
          {(items) =>
            items.map((fishing_material) => (
              <GridItem.Item
                key={fishing_material.code}
                from={Route.fullPath}
                to="$code"
                params={{code: fishing_material.code}}
              >
                <GridItem.Icon
                ...
                />
                <GridItem.Details>
                  <GridItem.Name>{fishing_material.name}</GridItem.Name>
                </GridItem.Details>
              </GridItem.Item>
            ))
          }
