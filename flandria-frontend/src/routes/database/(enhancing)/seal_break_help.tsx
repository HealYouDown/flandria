...
import {SealBreakHelpSort} from "@/gql/graphql"
...
type AllowedSortOptions = keyof SealBreakHelpSort
  ...
  queryOptions({
    ...
    queryFn: async () =>
      gqlClient.request(query, {
        ...
        order_by: [{[sortKey]: sortDirection} as unknown as SealBreakHelpSort],
      }),
  })

export const Route = createFileRoute("/database/(enhancing)/seal_break_help")({
  loaderDeps: ({
...
export const sealBreakHelpBreadcrumbItems: BreadcrumbItem[] = [
  ...databaseBreadcrumbItems,
  {
    label: "Seal Break Help Items",
    href: "/database/seal_break_help",
  },
]
...
function RouteComponent() {
  ...
  const {
    data: {
      all_seal_break_help_items: {total_count, items},
    },
  } = useSuspenseQuery(
  ...
  return (
    <>
      <PageHeader>
        <PageHeaderBreadcrumbs items={sealBreakHelpBreadcrumbItems} />
        <PageTitle title={last(sealBreakHelpBreadcrumbItems)!.label} />
      </PageHeader>
      ...
      <PaginationContentWrapper>
        ...
        <GridItemsList items={items}>
          {(items) =>
            items.map((seal_break_help_item) => (
              <GridItem.Item
                key={seal_break_help_item.code}
                from={Route.fullPath}
                to="$code"
                params={{code: seal_break_help_item.code}}
              >
                <GridItem.Icon
                  iconName={seal_break_help_item.icon}
                  itemGrade={seal_break_help_item.grade}
                />
                <GridItem.Details>
                  <GridItem.Name>{seal_break_help_item.name}</GridItem.Name>
                </GridItem.Details>
              </GridItem.Item>
            ))
          }
