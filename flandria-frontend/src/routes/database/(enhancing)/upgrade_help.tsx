...
import {UpgradeHelpSort} from "@/gql/graphql"
...
type AllowedSortOptions = keyof UpgradeHelpSort
  ...
  queryOptions({
    ...
    queryFn: async () =>
      gqlClient.request(query, {
        ...
        order_by: [{[sortKey]: sortDirection} as unknown as UpgradeHelpSort],
      }),
  })

export const Route = createFileRoute("/database/(enhancing)/upgrade_help")({
  loaderDeps: ({
...
export const upgradeHelpBreadcrumbItems: BreadcrumbItem[] = [
  ...databaseBreadcrumbItems,
  {
    label: "Upgrade Help Items",
    href: "/database/upgrade_help",
  },
]
...
function RouteComponent() {
  ...
  const {
    data: {
      all_upgrade_help_items: {total_count, items},
    },
  } = useSuspenseQuery(
  ...
  return (
    <>
      <PageHeader>
        <PageHeaderBreadcrumbs items={upgradeHelpBreadcrumbItems} />
        <PageTitle title={last(upgradeHelpBreadcrumbItems)!.label} />
      </PageHeader>
      ...
      <PaginationContentWrapper>
        ...
        <GridItemsList items={items}>
          {(items) =>
            items.map((upgrade_help_item) => (
              <GridItem.Item
                key={upgrade_help_item.code}
                from={Route.fullPath}
                to="$code"
                params={{code: upgrade_help_item.code}}
              >
                <GridItem.Icon
                  iconName={upgrade_help_item.icon}
                  itemGrade={upgrade_help_item.grade}
                />
                <GridItem.Details>
                  <GridItem.Name>{upgrade_help_item.name}</GridItem.Name>
                </GridItem.Details>
              </GridItem.Item>
            ))
          }
