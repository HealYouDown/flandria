...
import {ShipFlagSort} from "@/gql/graphql"
...
type AllowedSortOptions = keyof ShipFlagSort
  ...
  queryOptions({
    ...
    queryFn: async () =>
      gqlClient.request(query, {
        ...
        order_by: [{[sortKey]: sortDirection} as unknown as ShipFlagSort],
      }),
  })

export const Route = createFileRoute("/database/(ship)/(routes)/ship_flag")({
  loaderDeps: ({
...
export const shipFlagBreadcrumbItems: BreadcrumbItem[] = [
  ...databaseBreadcrumbItems,
  {
    label: "Ship Flags",
...
function RouteComponent() {
  ...
  const {
    data: {
      all_ship_flags: {total_count, items},
    },
  } = useSuspenseQuery(
  ...
  return (
    <>
      <PageHeader>
        <PageHeaderBreadcrumbs items={shipFlagBreadcrumbItems} />
        <PageTitle title={last(shipFlagBreadcrumbItems)!.label} />
      </PageHeader>
          ...
          {(items) =>
            items.map((ship_flag) => (
              ...
              >
                ...
                />
                <GridItem.Details>
                  <GridItem.Name>{ship_flag.name}</GridItem.Name>
                </GridItem.Details>
              </GridItem.Item>
            ))
          }
