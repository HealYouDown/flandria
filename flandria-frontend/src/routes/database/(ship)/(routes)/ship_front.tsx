...
const makeQueryOptions = makeQueryOptionsFactory<
  ...
  ShipFrontFilter,
  ShipFrontSort
>({queryKeyPrefix: "allShipFronts", query: query})
...
export const shipFrontBreadcrumbItems: BreadcrumbItem[] = [
  ...databaseBreadcrumbItems,
  {
    label: "Ship Flags",
    href: "/database/ship_flag",
  },
]
...
export function RouteComponent() {
  ...
  const {
    data: {
      all_ship_fronts: {total_count, items},
    },
  } = useSuspenseQuery(
    makeQueryOptions(
      offset,
  ...
  return (
    <>
      <PageHeader>
        <PageHeaderBreadcrumbs items={shipFrontBreadcrumbItems} />
        <PageTitle title={last(shipFrontBreadcrumbItems)!.label} />
      </PageHeader>
      ...
      <PaginationContentWrapper>
        ...
        <GridItemsList items={items}>
          {(items) =>
            items.map((ship_front) => (
              <GridItem.Item
                key={ship_front.code}
                from="/database/ship_front"
                to="$code"
                params={{code: ship_front.code}}
              >
                <GridItem.Icon
                ...
                />
                <GridItem.Details>
                  <GridItem.Name duration={ship_front.duration}>
                    {ship_front.name}
                  </GridItem.Name>
                  <GridItem.Subs
                    subs={[
                      `Lv. ${ship_front.level_sea}`,
                      shipClassFromObject(ship_front),
                    ]}
                  />
                  <GridItem.Effects effects={ship_front.effects} />
                </GridItem.Details>
              </GridItem.Item>
            ))
          }
