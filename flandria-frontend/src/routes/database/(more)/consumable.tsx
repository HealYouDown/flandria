...
import {ConsumableSort} from "@/gql/graphql"
...
type AllowedSortOptions = keyof ConsumableSort
  ...
  queryOptions({
    ...
    queryFn: async () =>
      gqlClient.request(query, {
        ...
        order_by: [{[sortKey]: sortDirection} as unknown as ConsumableSort],
      }),
  })

export const Route = createFileRoute("/database/(more)/consumable")({
  loaderDeps: ({
...
export const consumableBreadcrumbItems: BreadcrumbItem[] = [
  ...databaseBreadcrumbItems,
  {
    label: "Consumables",
    href: "/database/consumable",
  },
]
...
function RouteComponent() {
  ...
  const {
    data: {
      all_consumables: {total_count, items},
    },
  } = useSuspenseQuery(
  ...
  return (
    <>
      <PageHeader>
        <PageHeaderBreadcrumbs items={consumableBreadcrumbItems} />
        <PageTitle title={last(consumableBreadcrumbItems)!.label} />
      </PageHeader>
          ...
          {(items) =>
            items.map((consumable) => (
              ...
              >
                ...
                />
                <GridItem.Details>
                  <GridItem.Name>{consumable.name}</GridItem.Name>
                  <GridItem.Subs
                    subs={[
                      `Lv. ${consumable.level_land}/${consumable.level_sea}`,
                    ]}
                  />
                </GridItem.Details>
              </GridItem.Item>
