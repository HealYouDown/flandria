...
import {BulletSort} from "@/gql/graphql"
...
type AllowedSortOptions = keyof BulletSort
  ...
  queryOptions({
    ...
    queryFn: async () =>
      gqlClient.request(query, {
        ...
        order_by: [{[sortKey]: sortDirection} as unknown as BulletSort],
      }),
  })

export const Route = createFileRoute("/database/(more)/bullet")({
  loaderDeps: ({
...
export const bulletBreadcrumbItems: BreadcrumbItem[] = [
  ...databaseBreadcrumbItems,
  {
    label: "Bullets",
    href: "/database/bullet",
  },
]
...
function RouteComponent() {
  ...
  return (
    <>
      <PageHeader>
        <PageHeaderBreadcrumbs items={bulletBreadcrumbItems} />
        <PageTitle title={last(bulletBreadcrumbItems)!.label} />
      </PageHeader>
          ...
          {(items) =>
            items.map((bullet) => (
              ...
              >
                ...
                />
                <GridItem.Details>
                  <GridItem.Name>{bullet.name}</GridItem.Name>
                </GridItem.Details>
              </GridItem.Item>
            ))
          }
