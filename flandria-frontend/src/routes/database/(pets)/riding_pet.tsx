...
import {RidingPetSort} from "@/gql/graphql"
...
type AllowedSortOptions = keyof RidingPetSort
  ...
  queryOptions({
    ...
    queryFn: async () =>
      gqlClient.request(query, {
        ...
        order_by: [{[sortKey]: sortDirection} as unknown as RidingPetSort],
      }),
  })

export const Route = createFileRoute("/database/(pets)/riding_pet")({
  loaderDeps: ({
...
export const ridingPetBreadcrumbItems: BreadcrumbItem[] = [
  ...databaseBreadcrumbItems,
  {
    label: "Riding Pets",
    href: "/database/riding_pet",
  },
]
...
function RouteComponent() {
  ...
  const {
    data: {
      all_riding_pets: {total_count, items},
    },
  } = useSuspenseQuery(
  ...
  return (
    <>
      <PageHeader>
        <PageHeaderBreadcrumbs items={ridingPetBreadcrumbItems} />
        <PageTitle title={last(ridingPetBreadcrumbItems)!.label} />
      </PageHeader>
      ...
      <PaginationContentWrapper>
        ...
        <GridItemsList items={items}>
          {(items) =>
            items.map((riding_pet) => (
              <GridItem.Item
                key={riding_pet.code}
                from={Route.fullPath}
                to="$code"
                params={{code: riding_pet.code}}
              >
                <GridItem.Icon
                ...
                />
                <GridItem.Details>
                  <GridItem.Name duration={riding_pet.duration}>
                    {riding_pet.name}
                  </GridItem.Name>
                </GridItem.Details>
              </GridItem.Item>
