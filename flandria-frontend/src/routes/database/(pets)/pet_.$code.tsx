...
import {booleanToDisplayString, nameWithDuration} from "@/utils/format-helpers"
...
import {petBreadcrumbItems} from "@/routes/database/(pets)/pet"
import {queryOptions, useSuspenseQuery} from "@tanstack/react-query"
import {createFileRoute, notFound} from "@tanstack/react-router"
import {useMemo} from "react"

const petDetailedDocument = graphql(`
...
const makeQueryOptions = (code: string) =>
  queryOptions({
    queryKey: ["pet", code],
    queryFn: async () => gqlClient.request(petDetailedDocument, {code}),
  })

export const Route = createFileRoute("/database/(pets)/pet/$code")({
  loader: ({params: {code}}) =>
    queryClient.ensureQueryData(makeQueryOptions(code)).then((resp) => {
      if (!resp.pet) throw notFound()
    }),
...
function RouteComponent() {
  ...
  const details = useMemo(
    () => [
      {
        label: "Needs Pocket Watches",
        value: booleanToDisplayString(!pet.is_unlimited),
        description: "Whether the pet needs pocket watches.",
      },
      ...getBaseMixinDetails(pet),
    ],
    [pet],
  )
    ...
    <>
      <PageHeader>
        <PageHeaderBreadcrumbs
          items={[
            ...petBreadcrumbItems,
            {label: pet.name, href: Route.fullPath},
          ]}
        />
        <PageTitle
          left={
          ...
          title={nameWithDuration(pet.name, pet.duration)}
        />
      </PageHeader>
