...
import {PetCombineHelpSort} from "@/gql/graphql"
...
type AllowedSortOptions = keyof PetCombineHelpSort
  ...
  queryOptions({
    ...
    queryFn: async () =>
      gqlClient.request(query, {
        ...
        order_by: [{[sortKey]: sortDirection} as unknown as PetCombineHelpSort],
      }),
  })

export const Route = createFileRoute("/database/(pets)/pet_combine_help")({
  loaderDeps: ({
...
export const petCombineHelpBreadcrumbItems: BreadcrumbItem[] = [
  ...databaseBreadcrumbItems,
  {
    label: "Pet Combine Help Items",
    href: "/database/pet_combine_help",
  },
]
...
function RouteComponent() {
  ...
  const {
    data: {
      all_pet_combine_help_items: {total_count, items},
    },
  } = useSuspenseQuery(
  ...
  return (
    <>
      <PageHeader>
        <PageHeaderBreadcrumbs items={petCombineHelpBreadcrumbItems} />
        <PageTitle title={last(petCombineHelpBreadcrumbItems)!.label} />
      </PageHeader>
      ...
      <PaginationContentWrapper>
        ...
        <GridItemsList items={items}>
          {(items) =>
            items.map((pet_combine_help_item) => (
              <GridItem.Item
                key={pet_combine_help_item.code}
                from={Route.fullPath}
                to="$code"
                params={{code: pet_combine_help_item.code}}
              >
                <GridItem.Icon
                  iconName={pet_combine_help_item.icon}
                  itemGrade={pet_combine_help_item.grade}
                />
                <GridItem.Details>
                  <GridItem.Name>{pet_combine_help_item.name}</GridItem.Name>
                </GridItem.Details>
              </GridItem.Item>
            ))
          }
