...
import {ShoesFilter, ShoesPaginationQuery, ShoesSort} from "@/gql/graphql"
...
const makeQueryOptions = makeQueryOptionsFactory<
  ShoesPaginationQuery,
  ShoesFilter,
  ShoesSort
>({queryKeyPrefix: "allShoes", query: query})
...
export const shoesBreadcrumbItems: BreadcrumbItem[] = [
  ...databaseBreadcrumbItems,
  {
    label: "Shoes",
    href: "/database/shoes",
...
export function RouteComponent() {
  ...
  return (
    <>
      <PageHeader>
        <PageHeaderBreadcrumbs items={shoesBreadcrumbItems} />
        <PageTitle title={last(shoesBreadcrumbItems)!.label} />
      </PageHeader>
          ...
          {(items) =>
            items.map((shoes) => (
              <GridItem.Item
                ...
                from="/database/shoes"
                to="$code"
                params={{code: shoes.code}}
              >
                <GridItem.Icon iconName={shoes.icon} itemGrade={shoes.grade} />
                <GridItem.Details>
                  <GridItem.Name duration={shoes.duration}>
                    {shoes.name}
                  </GridItem.Name>
                  <GridItem.Subs
                    subs={[
                      `Lv. ${shoes.level_land}/${shoes.level_sea}`,
                      characterClassStringFromObject(shoes),
                    ]}
                  />
                  <GridItem.Effects effects={shoes.effects} />
                </GridItem.Details>
              </GridItem.Item>
            ))
          }
