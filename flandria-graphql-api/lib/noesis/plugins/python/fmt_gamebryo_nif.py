...
def registerNoesisTypes():
	...
	noesis.addOption(handle, "-textures", "path to use for textures prefix", noesis.OPTFLAG_WANTARG)
	noesis.addOption(handle, "-nif", "nif filename, only relevant for kf conversion", noesis.OPTFLAG_WANTARG)
...
def nifConstructModelFromNif(nif):
	...
	# FLORENSIA FLORENSIA FLORENSIA (keywords to find this part lol)
	textures_path = noesis.optGetArg("-textures")

	for material in mdl.modelMats.matList:
		# material: "NoeMaterial"

		# Ignores all meshes that are hidden in NifSkope (based on flags)
		# which do not have a texture anways (and are only used for animation references?)
		# Requires -gltfdiscnoren flag for export to GLTF, which ignores all meshes
		# with skipRender set.
		if material.texName == "":
			material.setSkipRender(True)
		else: # prepend textures path

			# workaround for realm textures which are just item_not_<model_name>.tga
			# but model has "item_not_<modelname>_niftex_0"
			# FIXME: One-handed sword is still broken, texture only contains lines.. idk
			if material.texName.endswith("_niftex_0"):
				filename = material.texName.replace("_niftex_0", "")
				material.texName = "{0}.tga".format(filename)

			material.texName = os.path.join(textures_path, material.texName)
			if hasattr(material, "specTexName"): # _g.dds files
				material.specTexName = os.path.join(textures_path, material.specTexName)
...
def kfLoadModel(data, mdlList):
	# We expect a parameter -modelname if we're using a .kf file
	# instead of asking for a filedialog, we supply the model data directly
	# otherwise we would have to deal with a filedialog somehow
	input_fpath = rapi.getInputName()
	input_path = os.path.dirname(input_fpath)

	if not noesis.optWasInvoked("-nif"):
		raise ValueError("Expected parameter -nif")

	nif_fname = noesis.optGetArg("-nif")
	model_fpath = os.path.join(input_path, nif_fname)
	...
	with open(model_fpath, "rb") as fp:
		otherNifData = fp.read()
